    ...
    return (
        <Box sx={{ p: { xs: 2, sm: 3 } }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                <Typography variant="h5" fontWeight={600}>My Leave Requests</Typography>
                <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/teacher/leave/apply')}>
                    Apply for Leave
                </Button>
            </Box>
                    ...
                    {selectedLeave && (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                <Chip label={selectedLeave.leaveType} size="small" />
                            </Box>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                <Typography color="text.secondary">Duration:</Typography>
                                <Typography>{formatDate(selectedLeave.startDate)} - {formatDate(selectedLeave.endDate)}</Typography>
                            </Box>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Dialog open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)}>
                ...
                <DialogActions>
                    ...
                    <Button color="error" variant="contained" onClick={() => deleteConfirm && handleCancel(deleteConfirm)} disabled={cancelMutation.isPending}>
